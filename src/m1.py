...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        p1x = self.circle.center.x
        p1y = self.circle.center.y
        dist = math.sqrt(((point.x - p1x) ** 2) + ((point.y - p1y) ** 2))
        return dist
    ...
    def swell_or_shrink_repeatedly(self,
        ...
        for i in range(times_to_swell_or_shrink):
            ...
            self.draw()
            opp = amount_to_swell_or_shrink * -1
            self.swell_or_shrink_once(opp)
            self.draw
    ...
    def swallow(self, other_circle_changer):
        ...
        new_circle_changer = CircleChanger(x, y, (self.get_distance_from(other_circle_changer.circle.center)) / 2, 'red', colors)
